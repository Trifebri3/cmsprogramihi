...
<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 w-full">
    <div>
        <h2 class="font-extrabold text-2xl text-slate-900 leading-tight">
            Sunting Konten dan Halaman
        </h2>
        <p class="text-xs text-slate-400 mt-1">Sesuaikan tata letak, isi tulisan, dan lampiran dokumentasi halaman.</p>
    </div>
</div>
@endsection
...
<div class="py-6 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="bg-white border border-gray-150 rounded-3xl p-8 shadow-sm space-y-8">
        ...
        <form action="{{ route('cms.contents.update', $content->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8" id="content-form">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 pb-6 border-b border-slate-100">
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title', $content->title) }}" required class="w-full rounded-xl border-gray-250 text-sm focus:ring-indigo-500 bg-white" placeholder="Contoh: Aksi Tanam Pohon Berjalan Sukses">
                </div>

                <div>
                    <label for="slug" class="block text-xs font-black text-slate-700 uppercase mb-2">Slug URL</label>
                    <input type="text" name="slug" id="slug" value="{{ old('slug', $content->slug) }}" required class="w-full rounded-xl border-gray-255 text-sm focus:ring-indigo-500 bg-white placeholder-slate-350" placeholder="contoh-artikel-baru">
                </div>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pb-6 border-b border-slate-100">
                <div>
                    <label for="type" class="block text-xs font-black text-slate-700 uppercase mb-2">Tipe Konten (Tidak Dapat Diubah)</label>
                    <input type="text" value="{{ ucfirst($content->type) }}" readonly disabled class="w-full rounded-xl border-gray-255 text-sm bg-slate-50 font-bold text-slate-450 cursor-not-allowed">
                </div>
                ...
                <div>
                    ...
                    <select name="status" id="status" required class="w-full rounded-xl border-gray-205 text-sm focus:ring-indigo-500 bg-white font-bold">
                        <option value="draft" {{ $content->status === 'draft' ? 'selected' : '' }}>Simpan sebagai Draft</option>
                        @if(auth()->user()->can('publish', App\Models\Content::class))
                            <option value="published" {{ $content->status === 'published' ? 'selected' : '' }}>Terbitkan (Publish)</option>
                        @else
                            <option value="published" class="text-gray-300" disabled {{ $content->status === 'published' ? 'selected' : '' }}>Terbitkan (Akses Terkunci)</option>
                        @endif
                        <option value="archived" {{ $content->status === 'archived' ? 'selected' : '' }}>Arsip</option>
                    </select>
                </div>
            </div>

            @if($content->type === 'post' && $categories->count() > 0)
                <div class="pb-6 border-b border-slate-100">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 bg-slate-50 rounded-2xl border border-slate-100">
                        @php
                            $activeCategoryIds = $content->categories->pluck('id')->toArray();
                        @endphp
                        @foreach($categories as $category)
                            <label class="inline-flex items-center text-xs font-bold text-slate-700 cursor-pointer">
                                <input type="checkbox" name="categories[]" value="{{ $category->id }}" {{ in_array($category->id, $activeCategoryIds) ? 'checked' : '' }} class="rounded border-slate-200 text-indigo-600 focus:ring-indigo-500 shadow-sm">
                ...
                <div class="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-4 bg-slate-50 p-4 rounded-2xl border border-slate-150 shadow-sm">
                    ...
                    <div class="flex gap-2 items-center">
                        <select id="block-type-selector" class="h-10 py-2 px-3 rounded-xl border border-slate-200 text-xs font-bold bg-white focus:ring-indigo-500 text-slate-700 w-48">
                            ...
                            <option value="button">Tombol Aksi / Tautan Link</option>
...
<script>
    ...
    let blocks = @json($content->content_blocks);
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        blocks.forEach((block, i) => {
            if (!block.id) {
                block.id = 'b_' + block.type + '_' + i + '_' + Date.now();
            }
        });
    ...
    function renderBlocks() {
        ...
        blocks.forEach((block, index) => {
            ...
            if (block.type === 'hero') {
                ...
                if (block.cover_url) {
                    coverPreview = `
                        <div class="mt-2 w-32 aspect-video rounded-xl overflow-hidden border border-slate-200 bg-white">
                            <img src="${block.cover_url}" class="w-full h-full object-cover">
                        </div>
                    `;
                }
                ...
                if (block.images && block.images.length > 0) {
                    existingImagesHtml = `
                        <div class="mt-4 border-t border-slate-100 pt-3">
                            <label class="block text-[10px] font-black text-slate-500 uppercase mb-2">Foto Saat Ini (Centang untuk menghapus)</label>
                            <div class="grid grid-cols-2 md:grid-cols-4 gap-3 p-3 bg-slate-50 rounded-xl border border-slate-150">
                                ${block.images.map(img => `
                                    <div class="relative group aspect-video rounded-lg overflow-hidden border border-slate-200 bg-white">
                                        <img src="${img}" class="w-full h-full object-cover">
                                        <label class="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center cursor-pointer text-white font-bold text-[10px]">
                                            <input type="checkbox" name="delete_images_${block.id}[]" value="${img}" class="rounded border-slate-300 text-red-655 focus:ring-red-500 mr-1.5">
                                            Hapus Foto
                                        </label>
                                    </div>
                                `).join('')}
                ...
                fieldsHtml = `
                    <div class="space-y-4">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                <label class="block text-[10px] font-black text-slate-500 uppercase">Unggah File Foto Galeri Tambahan (Bisa Pilih Banyak)</label>
